...
(()=>{
  ...
  async function render(analysis,id){
    ...
    const measures=$('#diagMeasurements');if(measures){measures.replaceChildren();for(const m of detail.measurements||[]){p(measures,m.name,'strong');p(measures,`${m.value??''} ${m.unit||''}`);}}
  }
