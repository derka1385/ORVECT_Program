...
Les routes appliquent l’isolation garage dérivée de la session authentifiée, le rate limiting existant et permettent anonymisation/invalidation. Les rôles et appartenances sont vérifiés côté serveur. MFA/SSO et provisionnement externe restent nécessaires avant une exploitation professionnelle.
...
```bash
docker build -f backend/Dockerfile.test -t automotive-diagnostic-ai-backend-test .
...
- Vercel : définir `API_INTERNAL_URL=https://<backend-railway>/api` (ou `NEXT_PUBLIC_API_URL` comme fallback de build). Le navigateur appelle le proxy same-origin `/backend-api`.
